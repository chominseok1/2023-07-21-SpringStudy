	  	   ...
	  	   <table class="table">
	  	   	<tr>
	  	   	 ...
	  	   	 <td width=90% class="inline">
	  	   	  <input type=text name=id ref=id size=20 class="input-sm" v-model="id" @keyup="idCheck" v-bind:readonly="isDisabled">
<!-- 	  	   	  <input type=button value="중복체크" class="btn btn-sm btn-success" ref="id" v-model="id" @blur="idCheck"> -->
	  	   	...
	  	   	<tr>
	  	   	 ...
	  	   	 <td width=90% class="inline">
	  	   	  ...
	  	   	  @keyup="pwdValidate">
	  	   	  &nbsp;재입력:<input type=password name=pwd1 ref="pwd1" size=20 class="input-sm" @keyup="pwdValidate2" v-model="pwd1">
	  	   	  <p>{{pwdOk}}</p>
	  	   	 </td>
	  	   	</tr>
    ...
    new Vue({
    	...
    	methods:{
    		submitForm:function(e){
    			if(this.id && this.name && this.pwd && this.pwd1 && this.sex && this.email && this.birthday 
    					&& this.post && this.addr1 && this.phone1 && this.phone && !this.idOk && !this.emailOk)
    			{
    			   return true;	
    			}
    		...
    		idCheckValidate:function(id){
    			...
    			{
    				this.idOk='아이디는 6자리 ~ 12자리 이내로 입력'	
    				return;
    			}
    			else if(id_res.search(/\s/)!=-1)
    			 {
    				// 공백 찾기 
    				this.idOk='아이디는 공백 없이 입력'
    				return;
    			 }	
    			else if(num<0 || eng<0)
    			{
    		...
    		pwdValidate:function(){
    			...
    			{
    				this.pwdOk='비밀번호는 8자리~20자리 이내로 입력하세'
    				return;
    			}
    			...
    			{
    				this.pwdOk='비밀번호는 영문,숫자를 혼합해서 써야해요'
    				return;
    			}
    			else
    			 {
