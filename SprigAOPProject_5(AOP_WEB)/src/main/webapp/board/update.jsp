...
<%--
	...
	= 자바 = 5버전 (6버전 자바로 설정)
	-------------------------------------------
	객체 생성 ~ 객체 소멸(객체의 생명주기 관리) => 클래스 관리 (Component) 관리
									  Container
	DI (의존성 주입) : 스프링에서 동작을 위해 필요한 데이터를 전송
					클래스와 클래스간의 연관관계를 설정 (IoC)
					= setter DI => setXxx() (Set메소드)
	...
	AOP (공통모듈) : 프로그램에서 반복적으로 수행 
		...
		Annotaion 쓰는 방식
		  ...
		  @Controller : Model (페이지 관리) => 화면이동
		  @RestController : Model (데이터 관리) => 화면에 출력할 데이터를 전송
		   ---- 다른언어와 연동 (JavaScript,Kotlin)
		   		---------- 일반 문자열 , JSON, XML
		  ...
		  *** React/Vue/MSA(Spring Cloud)
		      -------------------------- CI/CD(배포 jenkins) 지속적인 통합/지속적인 배포
		  동작 순서 => 인터셉트 
		  ------
